...
| 翻译 | Claude Haiku（`claude-haiku-4-5`） | 一次批量翻译省成本 |
...
4. GitHub 上：
   ...
   - **Settings → Secrets and variables → Actions → New repository secret**：`ANTHROPIC_API_KEY` = 你的 key
...
- ❗ **菜名变化**：每日重新翻译，意味着每天 ~20-40 道菜送 Claude Haiku。成本极低（一次 < $0.01），但跑久了可以加翻译缓存（同 fi 名复用之前 zh）。
