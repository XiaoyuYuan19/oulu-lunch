...
| 抓取 | Juvenes 学生餐厅 → Jamix Cloud JSON API | 唯一可靠的 Oulu 学生餐厅数据源（lounaat.info 不收录校园餐） |
...
浏览器/PWA ──▶ GitHub Pages (静态)
            ...
            GitHub Actions (每日 cron)
                  │
                  ├── scraper/scrape.py
                  │     ├── Jamix Cloud JSON   (Juvenes 学生餐厅)
                  │     └── Gemini 2.5 Flash   (翻译批量, 免费档)
                  └── 写回 docs/data.json
```

### Jamix API 速查

- 列表端点：`https://fi.jamix.cloud/apps/menuservice/rest/haku/menu/<customer_id>/<kitchen_id>?lang=fi`
- Juvenes 公司 `customer_id = 93077`（Oulu/Tampere 等都用同一个）
- 当前已映射的 Oulu 学生餐厅 `kitchen_id`：
  - 49 = Mara · 69 = Foobar · 70 = Kerttu · 75 = Medisiina
- 找新餐厅：juvenes.fi/&lt;名字&gt;/ 页面源码搜 `anro=93077&k=<N>`
...
4. GitHub 上：
   - **Settings → Pages**：Source = `Deploy from a branch`，Branch = `main`，Folder = `/docs` → Save
     （GH Pages 只支持 `/`(root) 或 `/docs` 两个目录，所以静态文件必须放在 `docs/`）
...
- ❗ **Jamix JSON 结构**：依赖 `kitchenId → menuTypes → menus → days → mealoptions → menuItems`，结构稳定但日期匹配用 YYYYMMDD 整数。
- ❗ **Voltti 暂未抓到**：juvenes.fi/voltti 指向 k=70（=Kerttu），同一 kitchen，所以 Voltti 与 Kerttu 共享菜单。当前 yaml 只列 Kerttu 避免重复。
- ❗ **Juve 没接入**：juvenes.fi/juve 页面无 Jamix 链接（可能纯纸质菜单），暂跳过。
- ❗ **菜名变化**：每日重新翻译，意味着每天 ~20-40 道菜送 Gemini。免费档完全够用；以后想省调用可加翻译缓存（同 fi 名复用之前 zh）。
- ❗ **diet 代码混在菜名里**：Jamix 部分项目把 `G,veg` 等代码塞进 `name` 字段，scraper 用 `INLINE_DIET_RE` 剥掉。看到漏的可加规则。
